...
const regionalGeometry={sourceBounds:{x0:27,x1:143,y0:27,y1:117,z0:84,z1:188},sourcePatient:{dimensions:[160,160,273] as [number,number,number]},sourceAnchors:{headZ:225,pelvisZ:39,leftShoulderX:55,rightShoulderX:105}};
const screenRegistration={headY:12,pelvisY:58,leftShoulderX:36,rightShoulderX:64};
const visualCentreY=pa.startingField.top+(100-pa.startingField.top-pa.startingField.bottom)/2+pa.startingField.centreY,visualCentreX=pa.startingField.left+(100-pa.startingField.left-pa.startingField.right)/2+pa.startingField.centreX;
...
const expectedZ=225+(visualCentreY-12)/(58-12)*(39-225),expectedX=55+(visualCentreX-36)/(64-36)*(105-55);
if(Math.abs(mappedDefault.sourceCentreVoxel[2]-expectedZ)>1||Math.abs(mappedDefault.sourceCentreVoxel[0]-expectedX)>1)throw new Error('PA chest target must be registered from displayed patient landmarks into source CT landmarks');
