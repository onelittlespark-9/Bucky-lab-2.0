...
const state=createAcquisitionState({position:pa,patientSex:'male',bodyYawDeg:pa.pose.bodyYaw+7,patientXPercent:3,patientYPercent:-2,beamXPercent:pa.startingField.centreX+3,beamYPercent:pa.startingField.centreY+2,tubeAngleDeg:4,sidCm:180,kVp:125,mAs:2,collimation:pa.startingField}),options=projectionOptions(state);
...
if(options.rotationDeg!==7||!Number.isFinite(options.centreXPercent)||!Number.isFinite(options.centreYPercent)||Math.abs(options.centreXPercent!)<1||Math.abs(options.centreYPercent!)<1)throw new Error('DRR options must preserve relative patient rotation and mapped beam-to-patient displacement');
...
const regionalGeometry={sourceBounds:{x0:27,x1:143,y0:27,y1:117,z0:84,z1:188},sourcePatient:{dimensions:[160,160,273] as [number,number,number]}};
const mappedDefault=projectionOptions(createAcquisitionState({position:pa,patientSex:'male',bodyYawDeg:pa.pose.bodyYaw,patientXPercent:0,patientYPercent:0,beamXPercent:pa.startingField.centreX,beamYPercent:pa.startingField.centreY,tubeAngleDeg:0,sidCm:180,kVp:125,mAs:2,collimation:pa.startingField,regionalGeometry}));
if(!mappedDefault.sourceCentreVoxel)throw new Error('Regional X-ray acquisition must carry a source-patient CT target');
const expectedDefault:[number,number,number]=[(27+143)/2,(27+117)/2,(84+188)/2];
if(mappedDefault.sourceCentreVoxel.some((v,i)=>Math.abs(v-expectedDefault[i])>.001))throw new Error('Default PA chest light field must target the verified chest source bounds centre');
const highField=projectionOptions(createAcquisitionState({position:pa,patientSex:'male',bodyYawDeg:pa.pose.bodyYaw,patientXPercent:0,patientYPercent:0,beamXPercent:pa.startingField.centreX,beamYPercent:pa.startingField.centreY-12,tubeAngleDeg:0,sidCm:180,kVp:125,mAs:2,collimation:pa.startingField,regionalGeometry}));
if(!highField.sourceCentreVoxel||highField.sourceCentreVoxel[2]<=mappedDefault.sourceCentreVoxel[2]+25)throw new Error('Moving the light field superiorly must move the DRR target superiorly in source-patient z');
...
if(!lowField.sourceCentreVoxel||lowField.sourceCentreVoxel[2]>=mappedDefault.sourceCentreVoxel[2]-25)throw new Error('Moving the light field inferiorly must move the DRR target inferiorly in source-patient z');
