...
def main():
 ...
 for region,names in REGIONS.items():
  ...
  cm=dict(m);cm['caseId']=f"{m['caseId']}-{region}";cm['dimensions']=[crop.shape[2],crop.shape[1],crop.shape[0]];cm['volumeUrl']=f"/cases/regional/{a.sex}/{region}/volume.i16.gz";source_affine=np.asarray(m['canonical']['sourceAffine'],dtype=float);crop_affine=source_affine.copy();crop_affine[:3,3]=(source_affine@np.array([b['x0'],b['y0'],b['z0'],1.0]))[:3];cm['canonical']=dict(m['canonical']);cm['canonical']['affine']=crop_affine.tolist();cm['coverage']={'region':region,'sourceBounds':b,'sourcePatient':{'dimensions':dims,'spacingMm':sp,'originMm':[m['canonical']['sourceAffine'][0][3],m['canonical']['sourceAffine'][1][3],m['canonical']['sourceAffine'][2][3]]},'structures':labels,'method':'strict-skull-segmentation'if region=='head'else'verified-segmentation-bounds','modalities':(['ct']if region in CT else[])+(['xray-drr']if region in XRAY else[])};cm.pop('segmentations',None);(dest/'manifest.json').write_text(json.dumps(cm,indent=2));built[region]=cm['coverage']['modalities']
