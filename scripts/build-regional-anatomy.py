...
def main():
 ...
 for region,names in REGIONS.items():
  ...
  cm=dict(m);cm['caseId']=f"{m['caseId']}-{region}";cm['dimensions']=[crop.shape[2],crop.shape[1],crop.shape[0]];cm['volumeUrl']=f"/cases/regional/{a.sex}/{region}/volume.i16.gz";cm['coverage']={'region':region,'sourceBounds':b,'structures':labels,'method':'strict-skull-segmentation'if region=='head'else'verified-segmentation-bounds','modalities':(['ct']if region in CT else[])+(['xray-drr']if region in XRAY else[])};cm.pop('segmentations',None);(dest/'manifest.json').write_text(json.dumps(cm,indent=2));built[region]=cm['coverage']['modalities']
