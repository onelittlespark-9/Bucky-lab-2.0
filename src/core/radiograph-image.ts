...
function paint(canvas:HTMLCanvasElement,image:RadiographImage){
 canvas.width=image.width;canvas.height=image.height;const ctx=canvas.getContext('2d');if(!ctx)return;
 const rgba=new Uint8ClampedArray(image.width*image.height*4);for(let i=0;i<image.pixels.length;i++){const v=image.pixels[i];rgba[i*4]=rgba[i*4+1]=rgba[i*4+2]=v;rgba[i*4+3]=255}
 ctx.putImageData(new ImageData(rgba,image.width,image.height),0,0);canvas.dataset.rendered='true'
}
