...
export function createAcquisitionState(input:AcquisitionInput):AcquisitionState{
 ...
 const presetCx=preset.left+presetW/2+preset.centreX,presetCy=preset.top+presetH/2+preset.centreY,currentCx=input.collimation.left+currentW/2+input.beamXPercent,currentCy=input.collimation.top+currentH/2+input.beamYPercent;
 const centreX=(currentCx-presetCx)/presetW*100-input.patientXPercent,centreY=(currentCy-presetCy)/presetH*100-input.patientYPercent,beamW=clamp(currentW/presetW*100,4,100),beamH=clamp(currentH/presetH*100,4,100),maskCx=50+centreX,maskCy=50+centreY;
