...
export interface ScreenPatientRegistration{headY:number;pelvisY:number;leftShoulderX:number;rightShoulderX:number}
export interface SourcePatientAnchors{headZ:number;pelvisZ:number;leftShoulderX:number;rightShoulderX:number}
export interface RegionalGeometry{sourceBounds:{x0:number;x1:number;y0:number;y1:number;z0:number;z1:number};sourcePatient:{dimensions:[number,number,number]};sourceAnchors:SourcePatientAnchors}
...
const map=(v:number,a0:number,a1:number,b0:number,b1:number)=>Math.abs(a1-a0)<1e-6?(b0+b1)/2:b0+(v-a0)/(a1-a0)*(b1-b0);

/**
 * Build one acquisition geometry from the displayed light field and the canonical source patient.
 * When rendered-patient landmarks are available, screen coordinates are registered directly to
...
export function createAcquisitionState(input:AcquisitionInput):AcquisitionState{
 ...
 // Collimation describes beam size. Do not move the detector mask when the tube/light field is
 // re-centred; anatomical targeting is handled by sourceCentreVoxel below.
 const localCollimation={left:(100-beamW)/2,right:(100-beamW)/2,top:(100-beamH)/2,bottom:(100-beamH)/2};
 let sourceCentreVoxel:Vec3|undefined;
 if(input.regionalGeometry){
  const{sourceBounds:b,sourcePatient:s,sourceAnchors:a}=input.regionalGeometry,[dx,dy,dz]=s.dimensions,baseX=(b.x0+b.x1)/2,baseY=(b.y0+b.y1)/2,baseZ=(b.z0+b.z1)/2,r=input.screenRegistration;
  if(r){
   // Use actual rendered landmarks. Sorting makes the mapping robust to PA/AP mirroring.
   const screenL=Math.min(r.leftShoulderX,r.rightShoulderX),screenR=Math.max(r.leftShoulderX,r.rightShoulderX),ctL=Math.min(a.leftShoulderX,a.rightShoulderX),ctR=Math.max(a.leftShoulderX,a.rightShoulderX);
   const z=clamp(map(currentCy-input.patientYPercent,r.headY,r.pelvisY,a.headZ,a.pelvisZ),-dz*.25,dz*1.25);
   if(lateral){
    const y=clamp(baseY+screenDx/100*dy,-dy*.25,dy*1.25);sourceCentreVoxel=[baseX,y,z];
   }else{
    const x=clamp(map(currentCx-input.patientXPercent,screenL,screenR,ctL,ctR),-dx*.25,dx*1.25);sourceCentreVoxel=[x,baseY,z];
   }
  }else{
   // Fallback for validation/initial mount before the articulated-patient landmarks are measured.
   sourceCentreVoxel=lateral?[baseX,baseY+screenDx/100*dy,baseZ-screenDy/100*dz]:[baseX+screenDx/100*dx,baseY,baseZ-screenDy/100*dz];
  }
 }
