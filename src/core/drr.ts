...
export function projectVolume(v:TeachingVolume,o:ProjectionOptions){
 ...
 for(let j=0;j<height;j++)for(let i=0;i<width;i++){
  ...
  const u=((i+.5)/width-.5)*detW/(lateral?sy:sx)/magnification-centreX,vz=((j+.5)/height-.5)*detH/sz/magnification-centreY;let src:[number,number,number],det:[number,number,number];
