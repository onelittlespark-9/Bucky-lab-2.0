...
export function projectVolume(v:TeachingVolume,o:ProjectionOptions){
 ...
 for(let j=0;j<height;j++)for(let i=0;i<width;i++){
  ...
  const uMm=((i+.5)/width-.5)*detW/magnification-centreX*(lateral?sy:sx),zMm=((j+.5)/height-.5)*detH/magnification-centreY*sz;let src:[number,number,number],det:[number,number,number];
