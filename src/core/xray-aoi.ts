...
export function xrayAoiFor(p:RadiographicPosition):XrayAoiGeometry{
 const s=norm(`${p.region} ${p.projection}`);
 let aoi:XrayAoi='whole',w=35,h=43,z=1,focusX=50,focusY=50,landmark=p.region;
 ...
 else if(/hand/.test(s)){aoi='hand';w=24;h=30;z=3;focusY=72;landmark='Hand'}
 else if(/femur/.test(s)){aoi='femur';w=24;h=43;z=2;focusY=73;landmark='Femur'}
 ...
 else if(/foot|calcane/.test(s)){aoi='foot';w=24;h=30;z=3;focusY=96;landmark='Foot'}
 return{aoi,detectorWidthCm:w,detectorHeightCm:h,defaultZoom:z,focusX,focusY,landmark};
}

export function positionerCameraFor(p:RadiographicPosition){
 const g=xrayAoiFor(p);
 return{zoom:g.defaultZoom,focusX:g.focusX,focusY:g.focusY,aoi:g.aoi,label:g.landmark};
}
